...
local function curl_callback(exitCode, stdOut, stdErr)
    if exitCode == 0 then
        obj.task = nil
        obj.last_pic = hs.http.urlParts(obj.full_url).queryItems[1].id
        -- wrong 
        ...
        --
        local localpath = os.getenv("HOME") .. "/wallpapers/" .. hs.http.urlParts(obj.full_url).queryItems[1].id
...
local function bingRequest()
    ...
    hs.console.printStyledtext("stat: ",stat)
        if stat == 200 then
            if pcall(function() hs.json.decode(body) end) then
                ...
                if obj.last_pic ~= pic_name then
                    ...
                    --for debug
                    --cmdtext ="/usr/bin/curl".." -A".. " " .. user_agent_str .." ".. obj.full_url.." -o "..localpath 
                    --hs.console.printStyledtext("cmd: ", cmdtext)
                    obj.task:start()
                end
            end
        else
